...
def transform_events_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    events = log_construction_utils.rename_attribute(events, "tokenType", "tokenType_name", mappings["token_map"])
    if sensitive_events:
        activity_split_candidates = ["transfer tokens", "give approval to transfer tokens", "mint tokens", "burn tokens"]
        events = log_construction_utils.create_contract_sensitive_events(events, mappings, creations, contracts_dapp, activity_split_candidates)
        ...
        market_info = log_construction_utils.propagate_extraInfo(events)
        events = pd.merge(events, market_info, on='market', how='left')
        market_type_info = log_construction_utils.propagate_marketType(events)
...
def transform_calls_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    calls = log_construction_utils.initial_transformation_calls(calls, True, txs_reverted)
    calls = log_construction_utils.rename_attribute(calls, "Activity", "Activity", mappings["calls_map_dapp"])
    calls = log_construction_utils.label_contracts(calls, mappings, creations, contracts_dapp)
    ...
    if sensitive_events:
        ...
        calls = log_construction_utils.create_contract_sensitive_events(calls, mappings, creations, contracts_dapp, activity_split_candidates)
...
def transform_delegatecalls_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    dcalls = log_construction_utils.initial_transformation_calls(dcalls, True, txs_reverted)
    dcalls = log_construction_utils.rename_attribute(dcalls, "Activity", "Activity", mappings["delegatecalls_map_dapp"])
    dcalls = log_construction_utils.label_contracts(dcalls, mappings, creations, contracts_dapp)
    for col in ["orderId", "betterOrderId", "worseOrderId", "tradeGroupId"]:
    ...
    if sensitive_events:
        ...
        dcalls = log_construction_utils.create_contract_sensitive_events(dcalls, mappings, creations, contracts_dapp, activity_split_candidates)
        if market_info is not None and market_type_info is not None:
...
def transform_zero_value_calls_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    zcalls = log_construction_utils.initial_transformation_calls(zcalls, True, txs_reverted)
    zcalls = log_construction_utils.rename_attribute(zcalls, "Activity", "Activity", mappings["calls_zero_value_map_dapp"])
    zcalls = log_construction_utils.label_contracts(zcalls, mappings, creations, contracts_dapp)
    for col in ["orderId", "betterOrderId", "worseOrderId", "tradeGroupId"]:
    ...
    if sensitive_events:
        ...
        zcalls = log_construction_utils.create_contract_sensitive_events(zcalls, mappings, creations, contracts_dapp, activity_split_candidates)
        if market_info is not None and market_type_info is not None:
...
def transform_events_non_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    events = pickle.load(open(path, "rb"))
    events = log_construction_utils.initial_transformation_events(events, False, txs_reverted)
    events = log_construction_utils.rename_attribute(events, "Activity", "Activity", mappings["event_map_non_dapp"])
...
def transform_calls_non_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    calls = pickle.load(open(path, "rb"))
    calls = log_construction_utils.initial_transformation_calls(calls, False, txs_reverted)
    calls = log_construction_utils.rename_attribute(calls, "Activity", "Activity", mappings["calls_map_non_dapp"])
...
def transform_delegatecalls_non_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    dcalls = pickle.load(open(path, "rb"))
    dcalls = log_construction_utils.initial_transformation_calls(dcalls, False, txs_reverted)
    dcalls = log_construction_utils.rename_attribute(dcalls, "Activity", "Activity", mappings["delegatecalls_map_non_dapp"])
...
def transform_zero_value_calls_non_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    zcalls = pickle.load(open(path, "rb"))
    zcalls = log_construction_utils.initial_transformation_calls(zcalls, False, txs_reverted)
    zcalls = log_construction_utils.rename_attribute(zcalls, "Activity", "Activity", mappings["calls_zero_value_map_non_dapp"])
    if "functionName" in zcalls.columns:
        zcalls.drop("functionName", axis=1, inplace=True)
...
def transform_zero_value_delegatecalls_non_dapp(resources_dir, base_contract, min_block, max_block,
    ...
    zdcalls = log_construction_utils.initial_transformation_calls(zdcalls, False, txs_reverted)
    zdcalls = log_construction_utils.rename_attribute(zdcalls, "Activity", "Activity", mappings["delegatecalls_zero_value_map_non_dapp"])
...
def build_address_dict(resources_dir, log_folder, base_contract, min_block, max_block, node_url):
    ...
    address_dict = address_classification.create_address_dict(base_contract, log_folder, resources_dir, min_block, max_block, contracts_dapp, node_url, creations, mappings)
...
if __name__ == '__main__':
    ...
    transform_data(RESOURCES_DIR, LOG_FOLDER, CONFIG["base_contract"], CONFIG["min_block"], CONFIG["max_block"], toggles)
    build_address_dict(RESOURCES_DIR, LOG_FOLDER, CONFIG["base_contract"], CONFIG["min_block"], CONFIG["max_block"], CONFIG["node_url"])
    build_log()
